...
cnx = 1.6        #机场中文名与跑道号分割线x坐标
stawx = 2.5    #机场与天气区分割线x坐标
allx = 10.9    #x轴总长度

ax1.plot([cnx,cnx],[allrow,0],color='lightgrey',linewidth=0.5)    #机场中文名与跑道号分割线
ax1.plot([stawx,stawx],[allrow,0],color='dimgray',linewidth=1)    #机场与天气分割线
# for x in range(1,24):
...
for x in range(1,allrow):
    ...
    ax1.plot([stawx,allx],[x,x],color='darkslategrey',linewidth=0.5)    #画横向网格线
...
for sta,detail in data.items():
    x = x+1
    wy = allrow-x
    stainfo = sta.split(); starwy = stainfo[0:-1]
    ax1.text(0.1,wy+0.4,' '.join(starwy),ha='left', va='center',fontproperties=font, size=8)    #机场四字码及跑道号
    ax1.text(2.05,wy+0.4,stainfo[-1],ha='center', va='center',fontproperties=font, size=8, color='dimgrey')    #通告号
    ax1.text(2.6,wy+0.4,detail['notamContent'],ha='left', va='center',fontproperties=font, size=8)    #通告内容
    if detail['style'] == 'EST':
        fillcolor = 'orange'
    else:
        fillcolor = 'tomato'
